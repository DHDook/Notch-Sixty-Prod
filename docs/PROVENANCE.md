...
| Master volume, mute, and fixed-output keyboard volume | Specification-derived / original commercial implementation | Product requirements + proprietary realtime master-gain architecture + public Core Audio HAL and IOKit HID APIs | PR #24 independently implements device volume/mute observation where writable and a passive Consumer Control Volume Increment/Decrement listener for fixed-volume outputs. The HID path maps keys to the commercial software master, never seizes/suppresses/synthesizes/reposts input, treats Input Monitoring permission separately from audio-route validity, and does not reproduce the historical virtual/HAL driver implementation. |
